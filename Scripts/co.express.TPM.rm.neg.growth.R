...
# only redoing clusters for years 2017 and 2020 at SERC
# two negative growth values were at SERC
...
TPM_2017_Spring = TPM_ExprData[,c(1:16,18:21,108:113,115:127)] # 38 samples
TPM_2017_Summer = TPM_ExprData[,c(1,22:34,36:39,128:133,135:146)] # 35 samples
TPM_2017_Fall = TPM_ExprData[,c(1,40:54,56:59,147:152,154:166)] # 38 samples
TPM_2020_Spring = TPM_ExprData[,c(1,277,279:284,286:289,291,294:295)] # 14 samples

# split TPMS for each year by each season by each site
# only for 2017 and 2018 where we have both sites

TPM_2017_Spring_SERC = TPM_2017_Spring[,c(1,21:39)]
TPM_2017_Summer_SERC = TPM_2017_Summer[,c(1,19:36)]
TPM_2017_Fall_SERC = TPM_2017_Fall[,c(1,21:39)]
...
#### Did Not DO: Soft Threshold Plotting ####
...
adjacency_2017_Fall = adjacency(TPM_2017_Fall_30, power = 9,type = "signed hybrid")
...
adjacency_2020_Spring = adjacency(TPM_2020_Spring_30, power = 6,type = "signed hybrid")
# make TOMS below with the above adjacney matrices first and remove to save space.

adjacency_2017_Fall_SERC = adjacency(TPM_2017_Fall_SERC_30, power = 7,type = "signed hybrid")
adjacency_2017_Spring_SERC = adjacency(TPM_2017_Spring_SERC_30, power = 10,type = "signed hybrid")
adjacency_2017_Summer_SERC = adjacency(TPM_2017_Summer_SERC_30, power = 10,type = "signed hybrid")
...
#### Growth Models ####
##### Spring 2017 ######
Spring.2017.MEs = read.csv("./Data/new.MEs/Spring.2017.MEs.no.neg.growth.csv") %>%
...
##### Summer 2017 ######
Summer.2017.MEs = read.csv("./Data/new.MEs/Summer.2017.MEs.no.neg.growth.csv") %>%
  mutate(growth.log = log(growth))
...
##### Fall 2017 #####
Fall.2017.MEs = read.csv("./Data/new.MEs/Fall.2017.MEs.no.neg.growth.csv") %>%
  mutate(growth.log = log(growth))
...
##### Spring 2020 #####
Spring.2020.MEs = read.csv("./Data/new.MEs/Spring.2020.MEs.no.neg.growth.csv")%>%
  mutate(growth.log = log(growth))
...
##### Spring SERC 2017 #####
Spring.2017.SERC.MEs = read.csv("./Data/new.MEs/Spring_SERC.2017.MEs.no.neg.growth.csv")%>%
  mutate(growth.log = log(growth))
...
filtered_Spring.2017.SERC.MEs.3 <- Spring.2017.SERC.MEs.3 %>%
  ...
  filter(fdr < .01)
##### Summer SERC 2017 #####
Summer.2017.SERC.MEs = read.csv("./Data/new.MEs/Summer_SERC.2017.MEs.no.neg.growth.csv")%>%
  mutate(growth.log = log(growth))
...
filtered_Summer.2017.SERC.MEs.3 <- Summer.2017.SERC.MEs.3 %>%
  ...
  filter(fdr < .01)
##### Fall SERC 2017 #####
Fall.2017.SERC.MEs = read.csv("./Data/new.MEs/Fall_SERC.2017.MEs.no.neg.growth.csv")%>%
  mutate(growth.log = log(growth))
